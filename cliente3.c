...
do{ 
  ...
  if (strcmp(buffer,"salir")==0){
			    if(write(descServer, buffer, strlen(buffer)+1)== -1){ perror("Write: "); return(0); }		
...
if(strcmp(buffer,"loguear")==0){ 
  ...
  bytes = read(descServer,buffer, TAMANIO); //el server envia "escriba su nombre de usuario (para loguearse)"
  printf(" %s\n", buffer);  
...
while(strcmp(buffer,"ok")!=0){   
     ...
     bytes = read(descServer,buffer, TAMANIO); //el server envia  ok o El usuario que ingreso no existe. Ingrese SU nombre de usuario:   
     printf("%s\n",buffer);
     
     }//recibe ok cuando se elija correctamente el usuario si no pide ingresar el nombre correcto
  
  
  printf("hola bienvenido ingrese su pasword recuerde que tiene 3 tiros\n");
