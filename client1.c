...
do{
    ...
    strcpy(buffer,"escriba salir para salir \n"); 
...
if(i>0){  
        printf("el usuario no existe se pide re ingresar usuario\n"); 
        strcpy(buffer,"Error el usuario no existe\t");
        if(write(descServer, buffer, strlen(buffer)+1)== -1){ perror("Write: "); return; } //envio re ingrese nombre usuario, no existe
        }
        
    strcpy(buffer,"Ingrese su nombre de usuario\n");
    if(write(descServer, buffer, strlen(buffer)+1)== -1){ perror("Write: "); return; } //envio ingrese nombre usuario
    bytes = read(descServer,buffer, TAMANIO); //recibo nombre usuario
  ...
  i++; 
  }while(strcmp(buffer,user->usuario)!=0) ;
