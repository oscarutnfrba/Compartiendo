...
do{
    ...
    printf(" envio a funcion %s\n", buffer); 
					  ...
					  printf("envio a loguearse\n");
                                          loguearse(&ppio);
					   printf("en el main despues de loguearse ok\n");
                                         }
...
//************************ingreso del resto de datos ********************************************
   strcpy(buffer,"Muy bien se ha establecido su contraseña.\nIngrese su mail.\n");
...
while(user==NULL){  
  ...
  if(i>0)  strcpy(buffer,"Error... no existe el usuario. Ingrese ''SU'' nombre de usuario:");   
  if (i==0) strcpy(buffer,"Ingrese su nombre de usuario:");   
  if(write(descCliente,buffer, strlen(buffer)+1)== -1) { perror("Write: ");  return ; } 
  bytes = read(descCliente,buffer, TAMANIO);    
  ...
  aux=*ppio;  
   ...
   printf("tengo la struct de => %s\n",user->usuario);
   
   bytes = read(descCliente,buffer, TAMANIO);  
     printf("recibi del cliente pass => ''%s''\n",buffer);  
  /*****************************   
                                 strcpy(buffer,"ok");
				 printf("copio a buffer =>%s y envio a cliente\n",buffer); 
  if(write(descCliente,buffer, strlen(buffer)+1)== -1) { perror("Write: ");  return ; } 
  */  //*****************************
  if(strcmp(buffer,user->pass)==0){                           
                                   strcpy(buffer,"ok"); 
                                   if(write(descCliente,buffer, strlen(buffer)+1)== -1) { perror("Write: ");  return ; }  
                                   return;//si el pass es correcto sale de loguearse y vuelve al main 
                                  }
 
  else{//si el pass no es correcto entra aca tiene tres intentos mas
       
   i=3; //veces que puede intentar ingresar el pass antes de ser desconectado
   
do{ 
   if(i==0){//intento demasiadas veces sale del programa
            if(write(descCliente,"exit", strlen(buffer)+1)== -1) { perror("Write: "); return ; } 
            close (descCliente); 
            printf("El cliente se desconecta. no conoce el pass\n");
	    return;
           }  
    if(i<3) {  
             if(write(descCliente,"error", strlen(buffer)+1)== -1) { perror("Write: "); return ; } 
            }
    
   if(i==3){
            if(write(descCliente,"Ingrese su pass", strlen(buffer)+1)== -1) { perror("Write: "); return ; } 
           } 
   ...
   }while(strcmp(pass,user->pass)!=0);
   
 strcpy(buffer,"ok");//si el pass esta bien envio ok al cliente 
 if(write(descCliente,buffer, strlen(buffer)+1)== -1) { perror("Write: "); return ; }
